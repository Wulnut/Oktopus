...
func TestDockerCompose_NoHardcodedSecrets(t *testing.T) {
	knownDefaults := []string{"oktopususer", "oktopuspw", "supersecretkey", "secretkey"}

	envFiles, _ := filepath.Glob(filepath.Join(composeDir, ".env.*"))

	// Also check the controller's .env
	controllerEnv := filepath.Join(projectRoot, "backend", "services", "controller", ".env")
	if _, err := os.Stat(controllerEnv); err == nil {
		envFiles = append(envFiles, controllerEnv)
	}
	...
	for _, envFile := range envFiles {
		...
		for _, secret := range knownDefaults {
			if strings.Contains(content, secret) {
				t.Errorf("File %s contains known default secret %q", filepath.Base(envFile), secret)
			}
		}
	}
}
...
func TestNginx_ApiBodySizeReasonable(t *testing.T) {
	...
	// Check global client_max_body_size
	globalRe := regexp.MustCompile(`(?m)^\s+client_max_body_size\s+(\S+);`)
	matches := globalRe.FindAllStringSubmatch(content, -1)

	for _, m := range matches {
		size := strings.TrimSpace(m[1])
		if size == "10G" || size == "10g" || size == "5G" || size == "5g" {
			t.Errorf("client_max_body_size %s is too large for a global/API setting -- should be <=10M for API endpoints", size)
		}
	}
}
